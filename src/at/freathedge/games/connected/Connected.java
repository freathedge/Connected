...
import at.freathedge.games.connected.util.renderer.MapRenderer;
...
public class Connected extends BasicGame {
    ...
    private final int loadingDuration = 5000;
    ...
    public void init(GameContainer gc) throws SlickException {
        ...
        player = new Player(mapHandler.getPlayerSpawnerX(), mapHandler.getPlayerSpawnerY(), map);
        camera = new Camera();
        camera.setMap(map);
        camera.zoom(3.5f);

        mapRenderer = new MapRenderer(map, camera, player, mapHandler);

        TrueTypeFont font = new TrueTypeFont(new java.awt.Font("Verdana", java.awt.Font.BOLD, 24), false);
        quitButton = new UIButton((gc.getWidth() - 200f) / 2, (gc.getHeight() - 150f) / 2, 200, 50, "Spiel beenden", font);
        resumeButton = new UIButton((gc.getWidth() - 200f) / 2, (gc.getHeight() - 250f) / 2, 200, 50, "Fortsetzen", font);
    ...
    public void update(GameContainer gc, int delta) throws SlickException {
        ...
        if (paused) {
            if (quitButton.isClicked(input)) gc.exit();
            if (resumeButton.isClicked(input)) paused = false;
            return;
        }
        ...
        if (leftClick) {
            float mouseWorldX = camera.getX() + input.getMouseX() / camera.getZoom();
            float mouseWorldY = camera.getY() + input.getMouseY() / camera.getZoom();
    ...
    public void render(GameContainer gc, Graphics g) throws SlickException {
        if (loading) {
            ...
            for (int i = 0; i < loadingDotCount; i++) loadingText.append(".");

            try {
                TrueTypeFont font = new TrueTypeFont(new java.awt.Font("Press Start 2P", java.awt.Font.PLAIN, 24), false);
                g.setFont(font);
            } catch (Exception e) {
                e.printStackTrace();
            }
